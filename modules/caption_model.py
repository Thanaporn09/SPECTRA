...
class CaptionModel(nn.Module):
    ...
    def beam_search(self, init_state, init_logprobs, *args, **kwargs):
        ...
        def beam_step(logprobs, unaug_logprobs, beam_size, t, beam_seq, beam_seq_logprobs, beam_logprobs_sum, state):
            ...
            logprobs = logprobs.reshape(batch_size, -1, vocab_size)  # shape: [N, current_beam, V]
            if t == 0:
                assert logprobs.shape[1] == 1
                beam_logprobs_sum = beam_logprobs_sum[:, :1]
    
            candidate_logprobs = beam_logprobs_sum.unsqueeze(-1) + logprobs  # shape: [N, current_beam, V]
            ys, ix = torch.sort(candidate_logprobs.reshape(batch_size, -1), -1, True)
            ys, ix = ys[:, :beam_size], ix[:, :beam_size]
            beam_ix = ix // vocab_size  # which beam (N x beam_size)
            selected_ix = ix % vocab_size  # which token (N x beam_size)
            state_ix = (beam_ix + torch.arange(batch_size).to(beam_ix.device).unsqueeze(-1) * logprobs.shape[1]).reshape(-1)
    
            if t > 0:
                # Verify that the beams in beam_seq match the selected indices
            ...
            new_beam_logprobs_sum = beam_logprobs_sum.gather(1, beam_ix) + \
                                    logprobs.reshape(batch_size, -1).gather(1, ix)
            # Replace strict equality check with an allclose tolerance check
            assert torch.allclose(new_beam_logprobs_sum, ys, atol=1e-5), "Mismatch in beam logprobs sum"
            ...
            assert torch.allclose(_tmp_beam_logprobs, beam_logprobs, atol=1e-5), "Mismatch in beam logprobs"
        ...
        state_table = [[(s.clone() if s is not None else None) for s in init_state] for _ in range(group_size)]
