...
class BaseTrainer(object):
    ...
    def _print_best_to_file(self):
        ...
        record_path = os.path.join(self.args.save_dir, self.args.dataset_name+'_'+self.args.model+'.csv')
...
class Trainer_SPECTRA(BaseTrainer):
    def __init__(self, model, criterion, metric_ftns, optimizer, args, lr_scheduler, train_dataloader, val_dataloader,
                 test_dataloader):
        super(Trainer_SPECTRA, self).__init__(model, criterion, metric_ftns, optimizer, args)
    ...
    def _train_epoch(self, epoch, model_name):
        ...
        train_loss = 0
        self.model.train()
        for batch_idx, (images_id, images, reports_ids, reports_masks, cnv) in enumerate(self.train_dataloader):
            ...
            output = self.model(images, cnv, reports_ids, mode = 'train')
        ...
        with torch.no_grad():
            val_gts, val_res = [], []
            for batch_idx, (images_id, images, reports_ids, reports_masks, cnv) in enumerate(self.val_dataloader):
                images, reports_ids, reports_masks, cnv = images.to(self.device), reports_ids.to(
                    self.device), reports_masks.to(self.device), cnv.to(self.device)
        ...
        with torch.no_grad():
            test_gts, test_res = [], []
            for batch_idx, (images_id, images, reports_ids, reports_masks, cnv ) in enumerate(self.test_dataloader):
